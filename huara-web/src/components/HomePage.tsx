    ...
    <div className="flex flex-col gap-4 px-4 pt-4 pb-24">
      ...
      <div className="flex items-center justify-between md:mb-8">
        <div className="flex items-center gap-3">
          <img src={LOGO} alt="logo" className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white object-contain p-1" />
          <div>
            <p className="text-white/60 text-xs md:text-sm">¡Hola,</p>
            <p className="text-white font-bold text-sm md:text-xl leading-tight">{user.name}! 👋</p>
          </div>
        </div>
        <button
          onClick={onLogout}
          className="md:hidden text-white/40 text-xs px-3 py-1.5 rounded-lg"
      ...
      {/* Info Grid (Points + Actions) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Points Banner */}
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="rounded-2xl p-5 relative overflow-hidden md:col-span-2"
          style={{
        ...
        >
          ...
          <div className="flex items-start justify-between relative z-10">
            <div>
              ...
              <p className="text-4xl md:text-6xl font-black" style={{ color: tierColor }}>
        ...
        {/* Quick Actions (Grid on mobile, column/grid on desktop) */}
        <div className="grid grid-cols-2 gap-2 h-full">
          {[
            ...
            <button
              key={btn.label}
              onClick={btn.action}
              className="flex flex-col items-center justify-center gap-2 py-4 rounded-2xl transition-transform active:scale-95 hover:bg-white/5"
      ...
      <div className="md:mt-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-white font-bold text-base md:text-xl">Ofertas Exclusivas</h2>
          <span className="text-white/40 text-xs md:text-sm">3 activas</span>
        </div>
        <div className="flex md:grid md:grid-cols-3 gap-4 overflow-x-auto md:overflow-visible pb-2 md:pb-0 -mx-4 md:mx-0 px-4 md:px-0 scrollbar-none">
          {DEALS.map((deal, i) => (
            <motion.div
              key={deal.id}
              ...
              className="flex-shrink-0 w-48 md:w-full rounded-2xl overflow-hidden relative group cursor-pointer"
            ...
            >
              ...
              {deal.badge && (
                <div
                  className="absolute top-2 right-2 text-[10px] md:text-xs font-black px-2 py-0.5 rounded-full z-10"
              ...
              <div className="p-4">
                <p className="text-white font-bold text-sm md:text-base">{deal.title}</p>
                <p className="text-white/50 text-xs mt-1 leading-relaxed">{deal.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
